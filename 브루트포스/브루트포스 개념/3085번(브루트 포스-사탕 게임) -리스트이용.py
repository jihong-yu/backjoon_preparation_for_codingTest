    ...
    for k in range(n):
        ...
        for l in range(n - 1):
            if array[k][l] == array[k][l + 1]: #만약 같은 열의 사탕의 색이 같다면
                ...
                countArray.append(countRow) #증가시킨 값을 리스트에 초기화
    ...
    for k in range(n):
        ...
        for l in range(n - 1): 
            if array[l][k] == array[l + 1][k]: #만약 같은 행의 사탕의 색이 같다면
                ...
                countArray.append(countColumn) #그 증가시킨 값을 리스트에 대입
...
print(max(countArray)) #색이 같은 사탕개수 최대값을 출력
